...
// Global CSS to ensure exact A4 landscape styling and perfect Arabic typography
const getStyles = (fontRegBase64: string, fontBoldBase64: string) => `
  ...
  .pdf-page {
    width: 297mm;
    height: 209mm; /* Slightly under 210mm to prevent accidental blank pages */
    padding: 10mm;
  ...
  for (const hw of Object.keys(hwGroups)) {
    ...
    for (const ed of Object.keys(edGroups)) {
      ...
      for (let p = 0; p < totalPages; p++) {
        ...
        // Add Summary Table only on the last page of this educator's list
        if (p === totalPages - 1) {
...
export const exportConfirmationPdfs = async (children: any[], fontData: { regular: string, bold: string, logo: string }, asZip: boolean) => {
  if (!children?.length) {
  ...
  const fontBase64 = { regular: fontData.regular, bold: fontData.bold };

  // Generate PDF for each Health Center
  for (const hc of Object.keys(groups)) {
    const htmlString = generateHTML(groups[hc], fontBase64, fontData.logo);
  ...
  if (!asZip) {
      ...
      const htmlString = generateHTML(groups[firstHcKey], fontBase64, fontData.logo);
      const container = document.createElement('div');
      container.innerHTML = htmlString;
      container.style.position = 'absolute';
