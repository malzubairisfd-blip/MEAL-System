...
function AddTaskFormContent() {
    ...
    const onAddTaskSubmit = async (data: z.infer<typeof AddTasksFormSchema>) => {
        ...
        try {
            ...
            router.push(`/meal-system/project/plan?projectId=${selectedProjectId}`);
        ...
        <div className="space-y-6">
             <div className="flex justify-between items-center">
                ...
                <Button variant="outline" asChild>
                    <Link href={`/meal-system/project/plan?projectId=${selectedProjectId}`}>
...
function RecursiveTaskItem({ control, index, remove, parentPath, logframe, baseActivityNumber }: { control: any; index: number; remove: (index: number) => void; parentPath: string; logframe: Logframe | null; baseActivityNumber?: number; }) {
    ...
    if (parentPath) {
        // e.g., parentPath = `tasks.7.subTasks.0.subTasks`
        // We need to construct 8.1.1
        ...
        // Handle nested subTask indices
        for (let i = 1; i < pathParts.length; i++) {
            const subIndexPart = pathParts[i]; // e.g., ".0" or "" for the first subtask level
            if(subIndexPart) {
                const subIndex = parseInt(subIndexPart.substring(1), 10); // Remove leading '.'
                if(!isNaN(subIndex)){
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 {isTopLevel && logframe && (
                    <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                       <FormField control={control} name={`${currentPath}.outcome`} render={({ field }) => (
                           <FormItem>
                               <FormLabel className="text-foreground">Outcome</FormLabel>
                        ...
                        <FormField control={control} name={`${currentPath}.output`} render={({ field }) => (
                           <FormItem>
                               <FormLabel className="text-foreground">Output</FormLabel>
                               <Select onValueChange={field.onChange} value={field.value}>
                ...
                <FormField control={control} name={`${currentPath}.title`} render={({ field }) => (
                   <FormItem className="col-span-2">
                       <FormLabel className="text-foreground">{`Activity ${activityNumbering} Title`}</FormLabel>
                 ...
                 {hasSubTasks === 'no' && (
                    <div className="col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="space-y-2">
                         ...
                         <FormField control={control} name={`${currentPath}.progress`} render={({ field }) => (<FormItem><FormLabel className="text-foreground">Progress (%)</FormLabel><FormControl><Input type="number" min="0" max="100" {...field} className="w-24" /></FormControl><FormMessage /></FormItem>)} />
                    </div>
                 )}
                 <FormField
                    control={control}
                    ...
                    render={({ field }) => (
                        <FormItem className="col-span-2 space-y-3">
                            <FormLabel className="text-foreground">{`Include Activity ${activityNumbering}.1?`}</FormLabel>
                            <FormControl>
                                <RadioGroup
                                ...
                                >
                                    <FormItem className="flex items-center space-x-2 space-y-0">
                                        ...
                                        <FormLabel className="font-normal text-foreground">Yes</FormLabel>
                                    </FormItem>
                                    <FormItem className="flex items-center space-x-2 space-y-0">
                                        <FormControl><RadioGroupItem value="no" /></FormControl>
                                        <FormLabel className="font-normal text-foreground">No</FormLabel>
                                    </FormItem>
                                </RadioGroup>
                            </FormControl>
                            <FormMessage />
