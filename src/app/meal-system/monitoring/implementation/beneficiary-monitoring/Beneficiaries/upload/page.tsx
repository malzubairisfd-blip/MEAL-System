...
export default function UploadPage() {
  ...
  const getButtonText = useCallback(() => {
    ...
    const handleSaveToDatabase = useCallback(async (mode: 'skip' | 'replace' | 'check' = 'check') => {
        ...
        try {
            const enrichedData = await loadCachedResult();
            if (!enrichedData) throw new Error("Could not load processed data from cache.");

            const { enrichedRecords } = enrichData(enrichedData);
            ...
            const recordsToProcess = enrichedRecords.map(record => {
                ...
                // Add all enriched fields that are also in DB_COLUMNS
            ...
            if (mode === 'check') {
                ...
                if (count > 0) {
                    ...
                    await handleSaveToDatabase('replace'); // No duplicates, so just save all
                }
            } else { // 'skip' or 'replace'
