...
const DB_SAVE_CHUNK_SIZE = 200; // Batch size for saving to DB
type WorkerProgress = {
    status: string;
    progress: number;
    completed ? : number;
...
export default function UploadPage() {
    ...
    const [uniqueIdMapping, setUniqueIdMapping] = useState({
        fileCol: '',
        dbCol: ''
    });
    const [manualDbMapping, setManualDbMapping] = useState({
        ui: '',
        db: ''
    });
    ...
    const [duplicateInfo, setDuplicateInfo] = useState < {
        isOpen: boolean;count: number;totalInFile: number;records: any[]
    } > ({
        isOpen: false,
        count: 0,
        totalInFile: 0,
    ...
    const unmappedUiColumns = useMemo(() => {
        const cachedData = rawRowsRef.current.length > 0 ? {
    ...
    useEffect(() => {
        registerServiceWorker();
        const clusterWorker = new Worker(new URL("@/workers/cluster.worker.ts",
            import.meta.url), {
            type: "module"
        });
        const scoringWorker = new Worker(new URL("@/workers/scoring.worker.ts",
            import.meta.url), {
            type: "module"
        });
    ...
    const fetchSettingsAndRules = useCallback(async () => {
        try {
            const [settingsRes, rulesRes] = await Promise.all([fetch("/api/settings"), fetch("/api/rules", {
                cache: 'no-store'
            })]);
    ...
    const startClustering = useCallback(async () => {
        if (!clusterWorkerRef.current || !scoringWorkerRef.current) {
            toast({
                title: t("upload.toasts.workerNotReady")
            });
            return;
        }
        if (!rawRowsRef.current.length) {
            toast({
                title: t("upload.toasts.noData")
            });
            return;
        }
    ...
    const handleAutoMatch = useCallback(async () => {
        ...
        const newMapping = new Map < string,
            string > ();
        const usedDbCols = new Set < string > ();
        availableColumns.forEach(uiCol => {
            const matchedDbCol = DB_COLUMNS.find(dbCol => dbCol.toLowerCase().replace(/_/g, '') === uiCol.toLowerCase().replace(/_/g, '').replace(/\s/g, '') && !usedDbCols.has(dbCol));
            if (matchedDbCol) {
    ...
    const executeBatchSave = async (records: any[], mode: 'skip' | 'replace') => {
        ...
        try {
            ...
            toast({
                title: "Save Complete!",
                description: `Successfully processed ${total} records. Saved: ${savedCount}, Skipped: ${skippedCount}.`
            });
        } catch (error: any) {
            setSaveStatus('error');
            toast({
    ...
    const handleSaveToDatabase = useCallback(async () => {
        if (!selectedProjectId || !uniqueIdMapping.fileCol || !uniqueIdMapping.dbCol) {
            toast({
                ...
                description: "Please select a project and map the unique ID column before saving.",
                variant: "destructive"
            });
            return;
        }
        ...
        try {
            ...
            const recordsToProcess = enrichedRecords.map(record => {
                ...
                for (const key of DB_COLUMNS) {
                    if (record[key] !== undefined && newRecord[key] === undefined) newRecord[key] = record[key];
                }
            ...
            if (uniqueIds.length === 0) throw new Error("Cannot check for duplicates because the selected unique ID column is empty for all records.");
            const res = await fetch('/api/bnf-assessed', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
            ...
            if (count > 0) {
                ...
                await executeBatchSave(recordsToProcess, 'skip');
            }
        } catch (error: any) {
            setSaveStatus('error');
            toast({
    ...
    }, [selectedProjectId, uniqueIdMapping, dbColumnMapping, toast, projects, executeBatchSave]);
        ...
        clusters.forEach((cluster: any) => {
            (cluster.records || []).forEach((record: RecordRow) => {
                recordToEnrichedClusterMap.set(record._internalId!, {
                    ...cluster,
                    specificRecordScores: record // Keep the specific scores for this record
                });
            });
        });

        const enrichedRecords = allRecords.map((record: RecordRow) => {
            const enrichedClusterData = recordToEnrichedClusterMap.get(record._internalId!);
            if (!enrichedClusterData) {
                return record; // Not in a cluster
            }
            
            const specificScores = enrichedClusterData.specificRecordScores;

            return {
                ...record,
                ...specificScores, // Add all scores from scoring worker
                Generated_Cluster_ID: enrichedClusterData.Generated_Cluster_ID,
                Cluster_Size: (enrichedClusterData.records || []).length,
                Flag: 'Review',
                Max_PairScore: enrichedClusterData.Max_PairScore,
                confidenceScore: enrichedClusterData.confidenceScore,
                reasons: Array.isArray(enrichedClusterData.reasons) ? enrichedClusterData.reasons.join(',') : enrichedClusterData.reasons,
                groupDecision: enrichedClusterData.groupDecision,
                recordDecisions: enrichedClusterData.recordDecisions?.[record._internalId!],
                decisionReasons: enrichedClusterData.decisionReasons?.[record._internalId!],
                avgWomanNameScore: enrichedClusterData.avgWomanNameScore,
                avgHusbandNameScore: enrichedClusterData.avgHusbandNameScore,
                avgFinalScore: enrichedClusterData.avgFinalScore,
            };
        });
    ...
    return (<div className="space-y-6">
              ...
              <label htmlFor="file-upload" className="flex-1">
                <div className="flex items-center justify-center w-full h-32 border-2 border-dashed rounded-lg cursor-pointer hover:bg-muted">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center">
                        ...
                        {file ?
                        (<> <p className="font-semibold text-primary">{file.name}</p> <p className="text-xs text-muted-foreground"> {rawRowsRef.current.length > 0 ? `${rawRowsRef.current.length} ${t("upload.file.rowsDetected")}` : t("upload.file.reading")} </p> </>) 
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  <CardTitle>{isTranslationLoading ?
                    <Skeleton className="h-8 w-48" /> : t("upload.steps.2.title")}</CardTitle>
                  {isTranslationLoading ?
                    <Skeleton className="h-5 w-64 mt-2" /> : <CardDescription>{t("upload.steps.2.description")}</CardDescription>}
                </div>
      ...
      {file && (
        <Card>
          <CardHeader>
            <CardTitle>{isTranslationLoading ?
             <Skeleton className="h-8 w-48" /> : t("upload.steps.3.title")}</CardTitle>
            {isTranslationLoading ?
             <Skeleton className="h-5 w-64 mt-2" /> : <CardDescription>{t("upload.steps.3.description")}</CardDescription>}
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div className="flex flex-wrap gap-2">
                <Button onClick={() => startClustering()} disabled={!isMappingComplete ||
                    !isDataCached || isProcessing}>
                    {isProcessing ?
      ...
      {workerStatus === "done" && (
        <>
            <Card>
                <CardHeader>
                    <CardTitle>{isTranslationLoading ?
                    <Skeleton className="h-8 w-48" /> : t("upload.steps.4.title")}</CardTitle>
                    {isTranslationLoading ?
                    <Skeleton className="h-5 w-64 mt-2" /> : <CardDescription>{t("upload.steps.4.description")}</CardDescription>}
                </CardHeader>
                <CardContent>
                    ...
                    <SummaryCard icon={<Sigma className="h-4 w-4 text-muted-foreground" />} title={t("upload.results.avgClusterSize")} value={clusters.length > 0 ?
                    (clusters.flatMap((c) => c.records).length / clusters.length).toFixed(2) : 0} />
                    </div>
            ...
            <Card>
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="flex flex-col items-center gap-4">
                        <Button onClick={() => handleSaveToDatabase()} disabled={!isDbMappingReady || isSaving} size="lg">
                            {isSaving ?
                            <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <ArrowUp className="mr-2 h-4 w-4"/>}
                        ...
                        {isSaving && (
                            <div className="w-full">
                                ...
                                <p className="text-sm text-center mt-1 text-muted-foreground">
                                    {saveStatus === 'checking_duplicates' ? 'Checking for existing records...' : 'Saving...'} 
                                    {saveStatus === 'saving' && ` ${saveProgress.toFixed(0)}% (${saveStats.saved}/${saveStats.total})`}
                                </p>
                            </div>
                        )}
                    </div>
          ...
          <AlertDialogContent>
              <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                    ...
                    <br/><br/>
                    Choose 'Skip' to ignore the duplicate records from your file and keep the existing database entries untouched. New records will still be added.
                  </AlertDialogDescription>
              </AlertDialogHeader>
              <AlertDialogFooter className="sm:justify-between">
                  <AlertDialogCancel onClick={() => { setIsSaving(false); setSaveStatus('idle'); }}>Cancel Saving</AlertDialogCancel>
                  <div className="flex gap-2">
