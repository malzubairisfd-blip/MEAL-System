...
const useApiRequest = () => {
  return async (url: string, options: RequestInit) => {
    ...
    try {
      ...
      throw new Error(
        `Server returned invalid response with status ${response.status}.`
      );
    }
    if (!response.ok) {
      throw new Error(parsed.error || parsed.details || `Server error: ${response.status}`);
    }
    return parsed;
  };
};
...
export default function UploadPage() {
  ...
  const isDbMappingReady = useMemo(
    () => uniqueIdMapping.fileCol && uniqueIdMapping.dbCol,
    [uniqueIdMapping]
  );

  const unmappedUiColumns = useMemo(() => {
    const cachedData = rawRowsRef.current.length
      ? {
    ...
    const availableColumns = [
      ...new Set([...columns, ...clusterTopLevelKeys, ...clusterRecordKeys]),
    ];
  ...
  const handleAutoMatch = useCallback(async () => {
    ...
    const clusterRecords = cachedData?.clusters?.[0]?.records?.[0]
      ? Object.keys(cachedData.clusters[0].records[0])
      : [];
    const clusterTopLevel = cachedData?.clusters?.[0]
      ? Object.keys(cachedData.clusters[0])
      : [];
    const availableColumns = [
      ...new Set([...columns, ...clusterTopLevel, ...clusterRecords]),
    ];
  ...
  const handleSaveToDatabase = useCallback(async () => {
    ...
    try {
      ...
      const enrichData = (cachedData: any): { enrichedRecords: any[] } => {
        ...
        clusters.forEach((cluster: any) => {
          (cluster.records || []).forEach((record: RecordRow) => {
            ...
            const recordSpecificData = {
              ...
              parts:
                typeof record.parts === "object" ? JSON.stringify(record.parts) : record.parts,
              husbandParts:
        ...
        return {
          enrichedRecords: allRecords.map((record: RecordRow) => {
            const enrichedData = recordToEnrichedClusterMap.get(record._internalId!) || {};
            return {
              ...record,
              ...enrichedData,
      ...
      const uniqueIds = recordsToProcess
        .map((r) => r[uniqueIdMapping.dbCol])
        .filter(Boolean);
      if (uniqueIds.length === 0)
        throw new Error(
          "Cannot check for duplicates because the selected unique ID column is empty for all records."
        );
  ...
  }, [selectedProjectId, uniqueIdMapping, dbColumnMapping, toast, apiRequest]);
