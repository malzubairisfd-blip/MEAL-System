    ...
    try {
        ...
        records.forEach((record, index) => {
            if (!record._internalId) {
              record._internalId = record.internalId || `fallback_${index}`;
            }
            ...
            if (clusterId) {
                ...
                recordsByCluster[clusterId].records.push(record as RecordRow);
                if (record.reasons) {
...
const ReasonCard = ({
    ...
    <Button
        ...
        onClick={onClick}
        className={cn("flex flex-col items-center justify-center h-24 w-24 gap-2", isActive && "border-primary border-2")}
    >
...
const SmartphoneScreen = ({
  ...
  const exclusionReasonOptions = [
      { label: "عدم انطباق المعايير", value: "عدم انطباق المعايير على المستفيدة", icon: XCircle },
      { label: "تكرار في الاستفادة", value: "تكرار في الاستفادة مثقفة/مستفيدة", icon: FileText },
      { label: "انتقال السكن", value: "انتقال سكن وإقامة المستفيدة خارج منطقة المشروع", icon: UserCheck },
  ] as const;
        ...
        <Table className={cn("text-xs", validationError && "border-2 border-red-500 rounded-lg")}>
          ...
          <TableBody>
            {cluster.records.map((record, index) => {
              ...
              return (
                <TableRow key={record._internalId || index} className={cn(isActive && "bg-blue-100 dark:bg-blue-900/30")}>
                    ...
                    {cluster.recordDecisions?.[activeRecord._internalId!] === 'مستبعدة' && (
                        <div className="mt-4">
                             ...
                             <div className="flex flex-wrap items-center gap-2">
                                {exclusionReasonOptions.map(opt => (
                                    <ReasonCard
                                        key={opt.value}
                                        icon={opt.icon}
                                        label={opt.label}
