// src/app/meal-system/monitoring/implementation/enrollment/review/database/page.tsx
"use client";
...
import { Loader2, Search, ArrowLeft, Users, FileDown, Filter, ArrowUpAZ, ArrowDownAZ, Trash2, Edit, Plus, ChevronLeft, ChevronRight } from "lucide-react";
...
import { exportEnrollmentToExcel } from "@/lib/exportEnrollmentToExcel";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle>Edit Record: {record.bnf_name}</DialogTitle>
...
export default function EnrollmentDatabasePage() {
  ...
  const handleUpdateRecord = async (updatedData: DbRecord) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || `Failed to update record ${updatedData.id}.`);
      }

      toast({ title: "Success", description: `Record ${updatedData.id} has been updated.` });
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (globalSearchTerm) {
      ...
      filtered = filtered.filter(record =>
        ...
        String(record.bnf_name).toLowerCase().includes(lowercasedTerm) ||
        String(record.ed_name).toLowerCase().includes(lowercasedTerm)
      );
    }
  ...
  const handleDownload = () => {
    ...
    exportEnrollmentToExcel(filteredRecords, allColumns);
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h1 className="text-3xl font-bold">Enrollment Database</h1>
      ...
      <Card className="w-full">
        <CardHeader>
          ...
          <CardDescription>
            Displaying {paginatedRecords.length} of {filteredRecords.length} records from `enrollment-review.db`.
          </CardDescription>
          <div className="relative pt-4">
            <Search className="absolute left-3 top-1/2 h-4 w-4 text-muted-foreground -translate-y-1/2" />
            <Input
              placeholder="Search by ID, Beneficiary Name, or Educator Name..."
          ...
          {loading ? (
            ...
            <div className="text-center text-red-500">{error}</div>
             ...
             <div className="text-center text-muted-foreground py-10">
                <p>No records found in the enrollment database.</p>
            </div>
          ) : (
            <>
