...
const formSchema = z
  .object({
    bnf_has_cmam: z.enum(["نعم", "لا"], {
      errorMap: () => ({ message: "يرجى تحديد ما إذا كان هناك سوء تغذية." }),
    }),
  ...
  .superRefine((data, ctx) => {
    if (data.bnf_has_cmam === "نعم") {
      if (!data.bnf_preg_lec) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["bnf_preg_lec"],
          message: "يرجى تحديد حالة الحمل أو الرضاعة.",
        });
      }
      if (data.bnf_preg_lec === "حامل" && !data.preg_mon?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["preg_mon"],
      ...
      if (data.bnf_preg_lec === "مرضع" && !data.child_age?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["child_age"],
          message: "يرجى تحديد عمر الطفل الرضيع.",
        });
      }
      ...
      if (!data.go_health_center) {
        ctx.addIssue({
          ...
          message: "يرجى تحديد ما إذا زار المستفيد المركز الصحي.",
        });
      }
...
export default function ScreeningDataEntryPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([cmamData, hcData]) => {
        ...
        const uniqueEducators: Educator[] = Array.from(
          new Map(projectCmams.map((item: any) => [item.ED_ID, item])).values()
        );
  ...
  const moveToNextBeneficiary = useCallback(() => {
    ...
    if (currentIndex !== -1 && currentIndex < filteredBeneficiaries.length - 1) {
      ...
      toast({
        title: "End of List",
        description: "You have reviewed all beneficiaries for this educator.",
      });
      setSelectedBeneficiaryId(null);
    }
  }, [filteredBeneficiaries, selectedBeneficiaryId, form, toast]);

  const handleCmamDecision = useCallback(
    async (value: "نعم" | "لا") => {
      if (value === "لا" && selectedBeneficiaryId) {
        ...
        try {
          ...
          toast({
            ...
            description: "Beneficiary marked as not having malnutrition.",
          });
    ...
    [selectedBeneficiaryId, moveToNextBeneficiary, toast]
  );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" asChild>
            <Link href="/meal-system/monitoring/implementation/process/CMAM-cases/beneficiaries/screening/database">
              <Database className="mr-2 h-4 w-4" /> Database
            </Link>
          </Button>
          <Button variant="outline" asChild>
            <Link href="/meal-system/monitoring/implementation/process/CMAM-cases/beneficiaries/screening/export">
      ...
      <Card>
        <CardHeader>
          <CardTitle>1. Select Project & Educator</CardTitle>
        </CardHeader>
        <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <Select
            ...
            onValueChange={setSelectedEducatorId}
          ...
          >
            ...
            <SelectContent>
              {educators.map((e) => (
                <SelectItem key={e.ED_ID} value={e.ED_ID}>
                  {e.ED_NAME} ({e.ED_ID})
                </SelectItem>
              ))}
            </SelectContent>
        ...
        <Card className="lg:col-span-1">
          ...
          <CardContent>
            ...
            <ScrollArea className="h-96 mt-4 border rounded-md">
              <RadioGroup
                value={selectedBeneficiaryId ? String(selectedBeneficiaryId) : undefined}
                onValueChange={(id) => setSelectedBeneficiaryId(Number(id))}
                className="p-2"
              >
                ...
                {watchHasCmam === "نعم" && (
                  <>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>اختر المركز الصحي القريب</FormLabel>
                          <FormControl>
                            <Popover open={healthCenterPopoverOpen} onOpenChange={setHealthCenterPopoverOpen}>
                              <PopoverTrigger asChild>
                                ...
                                >
                                  {field.value
                                    ? healthCenters.find((hc) => hc.hc_name === field.value)?.hc_name
                                      ...
                                      {healthCenters.map((hc) => (
                                        ...
                                        >
                                          <Check
                                            className={cn(
                                              "mr-2 h-4 w-4",
                                              hc.hc_name === field.value ? "opacity-100" : "opacity-0"
                                            )}
                                          />
                                          {hc.hc_name}
                                        </CommandItem>
