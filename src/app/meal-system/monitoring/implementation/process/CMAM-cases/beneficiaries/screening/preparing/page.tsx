...
const STATUS_LABELS: Record<string, string> = {
  idle: "Idle",
  STEP_ONE: "Step One · Preparing schema",
...
export default function PreparingBeneficiariesCMAMListPage() {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {columns.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <GitCompareArrows />
              2. Auto & Manual Mapping
            </CardTitle>
            <CardDescription>Match file columns with the bnf‑CMAM schema.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
      ...
      <Card>
        <CardHeader>
          <CardTitle>3. Validate & Save</CardTitle>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button
            className="w-full"
          ...
          >
            ...
            {loading.saving ? `Processing (${STATUS_LABELS[status]})` : "Save to bnf-CMAM.db"}
          </Button>
