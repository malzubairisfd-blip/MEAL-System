...
const formSchema = z
  .object({
    ...
    child_cmam_type: z.enum(["سوء تغذية متوسطة", "سوء تغذية حاد"]).optional(),
  ...
  .superRefine((data, ctx) => {
    if (data.isExistingChild === "لا") {
      if (!data.child_first_name?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["child_first_name"],
          message: "يرجى إدخال اسم الطفل الجديد.",
        });
      }
      ...
      if (!data.new_child_age_mon?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["new_child_age_mon"],
          message: "يرجى إدخال عمر الطفل بالشهور.",
        });
      }
    }
    if (data.child_has_cmam === "نعم") {
      if (!data.child_cmam_type) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["child_cmam_type"],
          message: "يرجى تحديد نوع سوء التغذية.",
        });
      }
      ...
      if (!data.go_health_center) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["go_health_center"],
          message: "يرجى تحديد ما إذا ذهبت إلى المركز الصحي.",
        });
      }
      if (!data.disc_date_day) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["disc_date_day"],
          message: "يرجى تحديد يوم الكشف.",
        });
      }
      if (!data.disc_date_month) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["disc_date_month"],
          message: "يرجى تحديد شهر الكشف.",
        });
      }
      if (!data.disc_date_year) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["disc_date_year"],
          message: "يرجى تحديد سنة الكشف.",
        });
      }
      if (!data.near_health_center?.trim()) {
        ctx.addIssue({
          code: z.ZodIssueCode.custom,
          path: ["near_health_center"],
          message: "يرجى اختيار المركز الصحي القريب.",
        });
      }
    }
  });
...
export default function ChildScreeningDataEntryPage() {
  ...
  const handleProjectSelect = useCallback(
    async (projectId: string) => {
      ...
      setSelectedChildId("");
      form.reset({ isExistingChild: "نعم", muac: 7.0 });
      await fetchProjectData(projectId);
    },
    [fetchProjectData, form]
  );

  const beneficiariesForEducator = useMemo(() => {
    if (!selectedEducatorId) return [];
    let filtered = allBeneficiaries.filter(
      (b) => b.BENEF_CLASS_DESC === "مستفيدة" && b.ED_ID === selectedEducatorId
    );
  ...
  const childrenOfBeneficiary = useMemo(() => {
    ...
    let filtered = allChildren.filter(
      (c) => c.benef_id === selectedBeneficiary.BENEF_ID && c.cmam_qualify === "Qualified"
    );
    if (childSearch) {
      const ls = childSearch.toLowerCase();
      filtered = filtered.filter(
        (c) => String(c.child_id).toLowerCase().includes(ls) || c.child_name.toLowerCase().includes(ls)
      );
    }
    return filtered;
    ...
    try {
      ...
      if (data.isExistingChild === "نعم") {
        ...
        payload = {
          ...
          new_child_age_mon: parseInt(data.new_child_age_mon || "0", 10),
        };
      }

      payload.child_has_cmam = data.child_has_cmam;
      ...
      if (data.child_has_cmam === "نعم") {
        Object.assign(payload, {
          ...
          disc_date: data.disc_date_year
            ? `${data.disc_date_year}-${data.disc_date_month}-${data.disc_date_day}`
            : null,
    ...
    const timer = setTimeout(() => {
      if (
        ...
        fetch("/api/validate-name", {
          ...
          .then((res) => res.json())
          .then((data) => setValidationErrors(data.result || []));
      } else {
        setValidationErrors([]);
      }
    }, 500);
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto p-4 md:p-6 pb-24" dir="rtl">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-foreground">إدخال نتائج التحص (CMAM) للأطفال</h1>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle>تحديد المشروع والمُثقف</CardTitle>
          </CardHeader>
            ...
            <div className="space-y-2">
              ...
              <Popover open={educatorOpen} onOpenChange={setEducatorOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {selectedEducatorId
                      ? educators.find((e) => e.ED_ID === selectedEducatorId)?.ED_NAME
                      : "اختر المثقف..."}
                ...
                <PopoverContent className="w-[300px] p-0" align="end">
                  <Command>
                    <CommandInput placeholder="ابحث عن المثقف..." />
                    <CommandList>
                      <CommandEmpty>لم يتم العثور على مثقف.</CommandEmpty>
                      <CommandGroup>
                        {educators.map((ed) => (
                          <CommandItem
                            key={ed.ED_ID}
                            value={ed.ED_NAME}
                            onSelect={() => {
                              setSelectedEducatorId(ed.ED_ID);
                          ...
                          >
                            <Check
                              className={cn(
                                "mr-2 h-4 w-4",
                                selectedEducatorId === ed.ED_ID ? "opacity-100" : "opacity-0"
                              )}
                            />
                            {ed.ED_NAME} ({ed.ED_ID})
                          </CommandItem>
                      ...
                      {beneficiariesForEducator.map((b) => (
                        <TableRow
                          ...
                          className={cn(
                            ...
                            selectedBeneficiary?.id === b.id && "bg-primary/10"
                          )}
                          onClick={() => {
        ...
        <Card className="lg:col-span-2">
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleSave)} className="space-y-6">
              <CardHeader>
                <CardTitle>بيانات فحص الطفل</CardTitle>
              </CardHeader>
              <CardContent className="space-y-6">
                <FormField
                  control={form.control}
                  name="isExistingChild"
                  render={({ field }) => (
                    <FormItem className="bg-card p-4 rounded-lg border">
                      <FormLabel className="text-base font-semibold">
                        هل الطفل مسجل في قاعدة البيانات؟
                      </FormLabel>
                      <FormControl>
                        <RadioGroup
                        ...
                        >
                          <div className="flex items-center space-x-2 space-x-reverse">
                            ...
                            <Label htmlFor="yes" className="font-normal m-0 cursor-pointer">
                              نعم
                            </Label>
                          </div>
                          <div className="flex items-center space-x-2 space-x-reverse">
                            <RadioGroupItem value="لا" id="no" />
                            <Label htmlFor="no" className="font-normal m-0 cursor-pointer">
                              لا
                            </Label>
                          </div>
                        </RadioGroup>
                ...
                {watchIsExisting === "نعم" ? (
                    ...
                    <ScrollArea className="h-[200px] border rounded-md">
                      <Table>
                        <TableHeader className="bg-muted sticky top-0">
                          <TableRow>
                            <TableHead className="w-[50px]">تحديد</TableHead>
                            <TableHead>رقم الطفل</TableHead>
                            <TableHead>اسم الطفل</TableHead>
                          </TableRow>
                        </TableHeader>
                        <TableBody>
                          {childrenOfBeneficiary.length > 0 ? (
                            childrenOfBeneficiary.map((c) => (
                              <TableRow
                                key={c.id}
                                className={cn("cursor-pointer hover:bg-muted/50", selectedChildId === c.child_id && "bg-secondary/10")}
                            ...
                            <TableRow>
                              <TableCell colSpan={3} className="text-center text-yellow-600 h-24 bg-yellow-500/10">
                                لا يوجد الطفل ضمن هذه المستفيدة.
                              </TableCell>
                            </TableRow>
                          )}
                        </TableBody>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border rounded-lg bg-card">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>جنس الطفل</FormLabel>
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-6 pt-2">
                              <div className="flex items-center space-x-2 space-x-reverse">
                                <RadioGroupItem value="ذكر" id="m" />
                                <Label htmlFor="m" className="font-normal m-0 cursor-pointer">
                                  ذكر
                                </Label>
                              </div>
                              <div className="flex items-center space-x-2 space-x-reverse">
                                <RadioGroupItem value="أنثى" id="f" />
                                <Label htmlFor="f" className="font-normal m-0 cursor-pointer">
                                  أنثى
                                </Label>
                              </div>
                            </RadioGroup>
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>اسم الطفل الجديد</FormLabel>
                          <FormControl>
                            <Input placeholder="الاسم الأول فقط..." {...field} />
                          </FormControl>
                        </FormItem>
                      )}
                    />
                    ...
                    <FormField
                      control={form.control}
                      name="new_child_age_mon"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel>عمر الطفل بالأشهر</FormLabel>
                          <FormControl>
                            <Input type="number" min="6" max="59" placeholder="6 - 59" {...field} />
                          </FormControl>
                        </FormItem>
                      )}
                    />
                  ...
                  <div className="space-y-6 pt-6 border-t border-dashed">
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="bg-accent/10 p-4 rounded-lg border border-accent/20">
                          <FormLabel className="text-base font-semibold text-accent-foreground">
                            هل الطفل يعاني من سوء تغذية؟
                          </FormLabel>
                          <FormControl>
                            <RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-8 pt-3">
                              <div className="flex items-center space-x-2 space-x-reverse">
                                <RadioGroupItem value="نعم" id="c_yes" />
                                <Label htmlFor="c_yes" className="font-medium cursor-pointer m-0">
                                  نعم
                                </Label>
                              </div>
                              <div className="flex items-center space-x-2 space-x-reverse">
                                <RadioGroupItem value="لا" id="c_no" />
                                <Label htmlFor="c_no" className="font-medium cursor-pointer m-0">
                                  لا
                                </Label>
                              </div>
                            </RadioGroup>
                    ...
                    {watchHasCmam === "نعم" && (
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-card p-6 rounded-lg border shadow-sm">
                        <FormField
                          control={form.control}
                          name="child_cmam_type"
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>حالة الطفل حالياً</FormLabel>
                              <FormControl>
                                <RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-6 pt-2">
                                  <div className="flex items-center space-x-2 space-x-reverse">
                                    <RadioGroupItem value="سوء تغذية متوسطة" id="mam" />
                                    <Label htmlFor="mam" className="font-normal m-0 cursor-pointer">
                                      سوء تغذية متوسطة
                                    </Label>
                                  </div>
                                  <div className="flex items-center space-x-2 space-x-reverse">
                                    <RadioGroupItem value="سوء تغذية حاد" id="sam" />
                                    <Label htmlFor="sam" className="font-normal m-0 cursor-pointer">
                                      سوء تغذية حاد
                                    </Label>
                                  </div>
                                </RadioGroup>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>قياس المواءك: {field.value?.toFixed(1) || 7}</FormLabel>
                              <FormControl>
                                <Slider
                                  min={7}
                                  max={12.4}
                        ...
                        <FormField
                          control={form.control}
                          name="go_health_center"
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>هل ذهب إلى المركز الصحي؟</FormLabel>
                              <FormControl>
                                <RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-6 pt-2">
                                  <div className="flex items-center space-x-2 space-x-reverse">
                                    <RadioGroupItem value="نعم" id="gh_yes" />
                                    <Label htmlFor="gh_yes" className="font-normal m-0 cursor-pointer">
                                      نعم
                                    </Label>
                                  </div>
                                  <div className="flex items-center space-x-2 space-x-reverse">
                                    <RadioGroupItem value="لا" id="gh_no" />
                                    <Label htmlFor="gh_no" className="font-normal m-0 cursor-pointer">
                                      لا
                                    </Label>
                                  </div>
                                </RadioGroup>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem className="col-span-1 md:col-span-2">
                              <FormLabel>اختر المركز الصحي المحيط</FormLabel>
                              <Select onValueChange={field.onChange} value={field.value}>
                                <FormControl>
                                  <SelectTrigger>
                                ...
                                <SelectContent>
                                  {healthCenters.map((hc) => (
                                    <SelectItem key={hc.hc_id} value={hc.hc_name}>
                                      {hc.hc_name}
                                    </SelectItem>
                                  ))}
                                </SelectContent>
                    ...
                    <div className="pt-4 flex justify-end">
                      <Button type="submit" size="lg" className="w-full md:w-auto" disabled={loading.saving || !isFormValid}>
                        {loading.saving ? (
