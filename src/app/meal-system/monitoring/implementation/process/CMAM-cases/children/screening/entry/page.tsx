                            ...
                            <CardContent className="space-y-6">
                                ...
                                {watchIsExisting === 'نعم' ? (
                                            ...
                                            <Table>
                                                ...
                                                <TableBody>
                                                    {childrenOfBeneficiary.length > 0 ? childrenOfBeneficiary.map(c => (
                                                        ...
                                                        <TableRow><TableCell colSpan={3} className="text-center text-yellow-600 h-24 bg-yellow-500/10">لا يوجد أطفال مؤهلون لهذه المستفيدة.</TableCell></TableRow>
                                                    )}
                                                </TableBody>
                                            </Table>
                                        </ScrollArea>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 border rounded-lg bg-card">
                                        ...
                                        {validationErrors.length > 0 && (<div className="col-span-1 md:col-span-2 p-3 rounded bg-destructive/10 border border-destructive/30 text-destructive-foreground"><p className="font-semibold mb-1">يوجد ملاحظات على الاسم:</p><ul className="list-disc list-inside text-sm space-y-1">{validationErrors.map((err, i) => <li key={i}>{err}</li>)}</ul></div>)}
                                ...
                                {((watchIsExisting === 'نعم' && selectedChildId) || watchIsExisting === 'لا') && (
                                    <div className="space-y-6 pt-6 border-t border-dashed">
                                        ...
                                        {watchHasCmam === 'لا' && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-card p-6 rounded-lg border shadow-sm">
                                                <FormField control={form.control} name="muac" render={({ field }) => (
                                                    <FormItem><FormLabel>قياس المواك: {field.value || 12.5}</FormLabel>
                                                    <FormControl><Slider min={12.5} max={20} step={0.1} value={[field.value || 12.5]} onValueChange={(v) => field.onChange(v[0])} /></FormControl>
                                        ...
                                        {watchHasCmam === 'نعم' && (
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-card p-6 rounded-lg border shadow-sm">
                                                <FormField control={form.control} name="child_cmam_type" render={({ field }) => (<FormItem><FormLabel>حالة الطفل حاليا</FormLabel><FormControl><RadioGroup onValueChange={field.onChange} value={field.value} className="flex gap-6 pt-2">
                                                ...
                                                <FormField control={form.control} name="muac" render={({ field }) => (<FormItem><FormLabel>قياس المواك: {field.value || 7}</FormLabel><FormControl><Slider min={7} max={16} step={0.1} value={[field.value || 7]} onValueChange={(v) => field.onChange(v[0])} /></FormControl><FormMessage /></FormItem>)} />
