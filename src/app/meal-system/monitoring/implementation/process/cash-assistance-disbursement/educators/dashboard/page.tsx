// src/app/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/dashboard/page.tsx
"use client";
...
export default function EducatorsCashdisbursementDashboardPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    fetch(`/api/ed-cash-disbursement?projectId=${selectedProjectId}`)
      .then((res) => res.json())
  ...
  const {
    totalCycles, totalEducatorsInList, totalEducatorsCashed,
    totalEducatorsUncashed, totalPaymentAmount, totalCashedAmount, totalUncashedAmount
  } = useMemo(() => {
    let cycles = new Set();
    for (let i = 1; i <= 76; i++) {
    ...
    return {
      ...
      totalEducatorsInList: records.filter(r => Number(r.total_pay_list) > 0).length,
      totalEducatorsCashed: records.filter(r => Number(r.total_cashed_cnt) > 0).length,
      totalEducatorsUncashed: records.filter(r => Number(r.total_uncashed_cnt) > 0).length,
  ...
  const captureAndCacheImages = async () => {
    ...
    request.onsuccess = e => {
      ...
      tx.objectStore('images').put(images, `ed-cash-disbursement-${selectedProjectId}`);
      tx.oncomplete = () => {
  ...
  const downloadExcel = async () => {
     ...
     request.onsuccess = async (e) => {
        ...
        const getReq = tx.objectStore('images').get(`ed-cash-disbursement-${selectedProjectId}`);
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">Educators Cash Disbursement Dashboard</h1>
          <p className="text-sm text-muted-foreground">Visualize payment cycle performance.</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" asChild><Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/upload"><Upload className="mr-2 h-4 w-4"/>Upload</Link></Button>
          <Button variant="outline" asChild><Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/database"><Database className="mr-2 h-4 w-4"/>Database</Link></Button>
          <Button variant="outline" asChild><Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators"><ArrowLeft className="mr-2 h-4 w-4"/>Back to Hub</Link></Button>
        </div>
      </div>
      <Card>
              ...
              <Collapsible defaultOpen onOpenChange={()=>{}}>
                  ...
                  <CollapsibleContent>
                      <div ref={keyFiguresRef} className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
                        ...
                        <KPICard title="ED in Pay List" value={totalEducatorsInList.toLocaleString()} icon={<Users />} />
                        <KPICard title="ED Cashed" value={totalEducatorsCashed.toLocaleString()} icon={<UserCheck />} />
                        <KPICard title="ED Uncashed" value={totalEducatorsUncashed.toLocaleString()} icon={<UserMinus />} />
