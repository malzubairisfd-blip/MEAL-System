// app/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/upload/page.tsx
"use client";
...
const STATUS_LABELS: Record<string, string> = {
  idle: "Idle",
  STEP_ONE: "Step 1 · Educators Database Check",
...
export default function EducatorsDisbursementUploadPage() {
  ...
  const [dbUniqueIdCol, setDbUniqueIdCol] = useState("ed_id");
  ...
  const executeSave = useCallback(async (mode: "skip" | "replace") => {
    ...
    try {
      const response = await fetch("/api/ed-cash-disbursement", {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/ed-cash-disbursement", {
        method: "POST", headers: { "Content-Type": "application/json" },
  ...
  const paymentReady = !!selectedProjectId && !!file && !!fileUniqueIdCol && (Object.values(paymentMapping).includes("ed_id") || Object.values(uncashedMapping).includes("ed_id"));

  return (
    <div className="space-y-6 pb-10">
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold">Educators Disbursement Upload</h1>
          <p className="text-sm text-muted-foreground">Map data lists to database fields via a strict 7-step saving process.</p>
        </div>
        <div className="flex gap-2">
          <Button asChild><Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/dashboard"><Upload className="mr-2 h-4 w-4" />Dashboard</Link></Button>
          <Button asChild><Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/database"><Database className="mr-2 h-4 w-4" />Database</Link></Button>
        ...
        <CardContent className="space-y-4">
          ...
          {duplicateState.open && (
            <Card className="border border-destructive bg-destructive/10 p-4 space-y-3">
              <h3 className="font-bold text-destructive">Duplicate Educators Found</h3>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3 pt-6">
            ...
            <KeyFigureCard title="Educators in List" value={aggregatedStats.payCount} icon={<Users className="h-4 w-4" />} />
            <KeyFigureCard title="Educators Cashed" value={aggregatedStats.cashedCount} icon={<DollarSign className="h-4 w-4" />} />
            <KeyFigureCard title="Educators Uncashed" value={aggregatedStats.uncashedCount} icon={<Wallet className="h-4 w-4" />} />
