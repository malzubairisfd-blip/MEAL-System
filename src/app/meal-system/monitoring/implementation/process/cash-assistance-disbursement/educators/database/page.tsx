// src/app/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators/database/page.tsx
"use client";
...
export default function EducatorsDisbursementDatabasePage() {
    ...
    const filteredRecords = useMemo(() => {
        ...
        return records.filter(r => 
            String(r.ed_id).toLowerCase().includes(lowercasedTerm) ||
            String(r.ed_name).toLowerCase().includes(lowercasedTerm)
        );
    }, [records, searchTerm]);
    ...
    const educatorSummary = useMemo(() => {
        if (!selectedEducatorId) return null;
        const record = records.find(r => r.ed_id === selectedEducatorId);
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold">Educators Disbursement Database</h1>
                <div className="flex gap-2">
                    <Button variant="outline" asChild>
                        <Link href="/meal-system/monitoring/implementation/process/cash-assistance-disbursement/educators">
            ...
            <Card>
                <CardHeader>
                    <CardTitle>Educator Summary</CardTitle>
                    <CardDescription>Select a Educator to see their overall session summary.</CardDescription>
                </CardHeader>
                <CardContent>
                    <Select onValueChange={setSelectedEducatorId} value={selectedEducatorId || ''}>
                        <SelectTrigger className="w-full md:w-1/2">
                            <SelectValue placeholder="Select an Educator..." />
                        </SelectTrigger>
                        <SelectContent>
                            {records.map(r => (
                                <SelectItem key={r.id} value={r.ed_id}>
                                    {r.ed_name} ({r.ed_id})
                                </SelectItem>
                            ))}
                        </SelectContent>
                    ...
                    {educatorSummary && (
                        <div className="mt-7 border rounded-lg overflow-hidden">
                            <Table>
                                ...
                                <TableBody>
                                    <TableRow>
                                        <TableCell>{educatorSummary.total_pay_list}</TableCell>
                                        <TableCell>{educatorSummary.total_pay_cyc_cnt}</TableCell>
                                        <TableCell>{educatorSummary.total_pay_amt}</TableCell>
                                        <TableCell>{educatorSummary.total_cashed_cnt}</TableCell>
                                        <TableCell>{educatorSummary.total_cashed_amt}</TableCell>
                                        <TableCell>{educatorSummary.total_uncashed_cnt}</TableCell>
                                        <TableCell>{educatorSummary.total_uncashed_amt}</TableCell>
                                        <TableCell>{educatorSummary.final_comments}</TableCell>
                                    </TableRow>
                                </TableBody>
                            </Table>
                        </div>
                ...
                <CardHeader>
                    ...
                    <CardDescription>
                     ...
                     <div className="relative pt-4">
                        ...
                        <Input
                        placeholder="Search by Educator ID or Name..."
                        value={searchTerm}
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6" ref={phonePanelRef}>
                            <div className="lg:col-span-3">
                                <div className="bg-background rounded-[1rem] overflow-hidden">
                                    <div className="p-3 border-b text-center"><h3 className="font-bold">Educators Disbursement Records</h3></div>
