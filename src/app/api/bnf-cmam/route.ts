...
const columnDefs = DB_COLUMNS_FOR_CREATION.replace(/^\(|\)$/g, "")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);
  ...
  try {
    ...
    if (action === "get_record_count") {
        ...
        try {
            ...
            const result = dbInstance.prepare("SELECT COUNT(*) as count FROM bnf_cmam").get() as { count: number } | undefined;
    ...
    if (action === "check_duplicates") {
      ...
      const idsToCheck = Array.isArray(uniqueIds)
        ? uniqueIds.map((id) => normalizeLookupValue(id)).filter(Boolean)
        : [];
      let db: Database.Database | null = null;
      try {
        db = initializeDatabase();
        const totalExisting = (db.prepare("SELECT COUNT(*) as total FROM bnf_cmam WHERE project_id = ?").get(projectId) as { total: number })?.total || 0;
        if (totalExisting === 0 || !idsToCheck.length) {
          return NextResponse.json({ count: 0, totalInDb: totalExisting, duplicateIds: [] });
        }
        const existingIds = new Set<string>();
        const chunks = chunkArray(idsToCheck, 900);
        for (const chunk of chunks) {
          if (!chunk.length) continue;
          const placeholders = chunk.map(() => "?").join(",");
          const stmt = db.prepare(
            `SELECT "${lookupColumn}" FROM bnf_cmam WHERE project_id = ? AND "${lookupColumn}" IN (${placeholders})`
          );
    ...
    if (action === "save") {
      ...
      (async () => {
        ...
        try {
          ...
          const uniqueColumnFile = Object.entries(normalizedMapping).find(([, dbCol]) => dbCol === sanitizedUnique)?.[0] || "";
          if (!uniqueColumnFile) {
            sendProgress(writer, { type: "error", error: "Mapped unique file column not found." });
            writer.close();
            return;
          }
          ...
          const insertPlaceholders = insertColumns.map((col) => `@${col}`).join(", ");
          const insertStmt = db.prepare(`INSERT INTO bnf_cmam (${insertColumns.map((col) => `"${col}"`).join(",")}) VALUES (${insertPlaceholders})`);
          ...
          const duplicateSet = new Set(
            Array.isArray(duplicateIds)
              ? duplicateIds.map((id) => normalizeLookupValue(id)).filter(Boolean)
              : []
          );

          const transaction = db.transaction((rowsToProcess: any[]) => {
            for (const row of rowsToProcess) {
              ...
              const uniqueValue = normalizeLookupValue(payload[sanitizedUnique]);
              if (!uniqueValue) {
                stats.skipped++;
                continue;
              }
              if (mode === "skip" && duplicateSet.has(uniqueValue)) {
                stats.skipped++;
                continue;
              }
          ...
          if (regDateParsed && currDateParsed) {
            ...
            db.transaction((rowsToUpdate: any[]) => {
              rowsToUpdate.forEach((record) => {
                ...
                updateDateStmt.run(regDate, currDate, diffDays.toFixed(2), diffMonths.toFixed(2), record.id);
              });
            })(regRecords);
          }
...
export async function DELETE(req: Request) {
    try {
        ...
        if (!Array.isArray(ids) || ids.length === 0) {
            return NextResponse.json({ error: "Expected an array of record IDs" }, { status: 400 });
        }

        const placeholders = ids.map(() => '?').join(',');
        const stmt = db.prepare(`DELETE FROM bnf_cmam WHERE id IN (${placeholders})`);
        
        const info = stmt.run(...ids);
        db.close();
        
        return NextResponse.json({ message: `${info.changes} records deleted.` });
    } catch (err: any) {
