...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (action === "create_new_child") {
      ...
      let bnfDb: Database.Database | null = null;
      let projectsDb: Database.Database | null = null;

      try {
        childDb = initializeDatabase();
        bnfDb = new Database(getBnfCmamDbPath(), { fileMustExist: true });
        projectsDb = new Database(getProjectsDbPath(), { fileMustExist: true });

        const project = projectsDb.prepare("SELECT projectName FROM projects WHERE projectId = ?").get(payload.project_id) as any;
        const bnf = bnfDb.prepare("SELECT * FROM bnf_cmam WHERE BENEF_ID = ?").get(payload.benef_id) as any;
        ...
        const stmt = childDb.prepare(`
          INSERT INTO child_cmam (
        ...
        stmt.run({
          ...
          ed_id: bnf.ED_ID,
          ed_name: bnf.ED_NAME,
          ed_phone: bnf.ed_phone,
          gov_name: bnf.GOV_NAME,
          mud_name: bnf.MUD_NAME,
          ozla_name: bnf.OZLA_NAME,
          vill_name: bnf.VILL_NAME,
          BENEF_CLASS_DESC: bnf.BENEF_CLASS_DESC,
        });

        return NextResponse.json({ success: true, message: "New child saved successfully" });
      } finally {
        if (childDb) childDb.close();
        if (bnfDb) bnfDb.close();
    ...
    if (action === "update_child") {
      ...
      try {
        childDb = initializeDatabase();
        const stmt = childDb.prepare(`
          UPDATE child_cmam SET 
            child_has_cmam = @child_has_cmam, child_cmam_type = @child_cmam_type, muac = @muac,
            go_health_center = @go_health_center, disc_date = @disc_date, 
            near_health_center = @near_health_center, hw_id = @hw_id, hw_name = @hw_name, 
            hc_id = @hc_id, hc_name = @hc_name
