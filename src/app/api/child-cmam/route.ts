...
const DB_SCHEMA = `(
  ...
  attend_hc TEXT, conf_date TEXT, child_has_cmam_hc TEXT, hc_card_no TEXT, meas_type TEXT,
  zscore_h REAL, zscore_w REAL, zscore REAL, child_cmam_cond TEXT, exp_start_treat_date TEXT, exp_end_treat_date TEXT,
  ...
  zscore_c2 REAL, cmam_result_c2 TEXT, not_attend_reason_c2 TEXT, child_age_c2 TEXT,
  cure_rate_c2 TEXT, positive_c2 TEXT, negative_c2 TEXT, next_cycle_c2 TEXT,
  child_attend_c3 TEXT, child_isprev_ref_c3 TEXT, date_attend_c3 TEXT, child_has_cmam_c3 TEXT,
  child_cmam_cond_c3 TEXT, meas_type_c3 TEXT, muac_c3 REAL, zscore_h_c3 REAL, zscore_w_c3 REAL,
  zscore_c3 REAL, cmam_result_c3 TEXT, not_attend_reason_c3 TEXT, child_age_c3 TEXT,
  cure_rate_c3 TEXT, positive_c3 TEXT, negative_c3 TEXT, next_cycle_c3 TEXT,
  data JSON
)`;

const columnDefs = DB_SCHEMA.replace(/[()]/g, "")
  .split(",")
  .map((s) => s.trim().split(/\s+/)[0].replace(/"/g, ""));
const DB_COLUMNS = Array.from(
  new Set(columnDefs.filter((c) => c && c.trim() !== ""))
);
...
function initializeDatabase() {
  ...
  db.exec(
    `CREATE UNIQUE INDEX IF NOT EXISTS idx_child_project_id ON child_cmam (project_id, child_id)`
  );
  return db;
}
...
const normalizeBenefNoValue = (value: unknown): string => {
  if (value === undefined || value === null || value === "") return "";
  const stringValue = String(value).trim();
  // Remove trailing .0 or .00 etc. from Excel number formatting
  ...
  try {
    ...
    const records =
      projectId && projectId !== "all"
        ? db
            .prepare("SELECT * FROM child_cmam WHERE project_id = ?")
            .all(projectId)
  ...
  try {
    ...
    if (action === "create_new_child") {
      ...
      try {
        const countResult = db
          .prepare("SELECT COUNT(*) as count FROM child_cmam WHERE benef_id = ?")
          .get(benef_id) as { count: number };
        const newChildIdx = (countResult?.count || 0) + 1;

        bnfCmamDb = new Database(getBnfCmamDbPath(), { fileMustExist: true });
        const parentRecord: any = bnfCmamDb
          .prepare("SELECT * FROM bnf_cmam WHERE BENEF_ID = ?")
          .get(benef_id);
        if (!parentRecord)
          throw new Error("Parent beneficiary record not found in bnf-cmam.db");
        ...
        newRecord.new_child_age_years = (
          Number(updates.new_child_age_mon) / 12
        ).toFixed(2);
        newRecord.cmam_qualify = "Qualified";

        const insertCols = Object.keys(newRecord).filter((k) =>
          DB_COLUMNS.includes(k)
        );
        const placeholders = insertCols.map((c) => `@${c}`).join(", ");
        const stmt = db.prepare(
          `INSERT INTO child_cmam (${insertCols.join(
            ", "
          )}) VALUES (${placeholders})`
        );
        ...
        return NextResponse.json({
          success: true,
          message: "New child record created.",
        });
      } finally {
        db.close();
    ...
    if (action === "check_duplicates") {
      ...
      try {
        ...
        const totalInDb = (
          db
            .prepare("SELECT COUNT(*) as count FROM child_cmam WHERE project_id = ?")
            .get(projectId) as { count: number }
        ).count;
        const uniqueIds = records
          .map(
            (r: any) =>
              `${normalizeBenefNoValue(r[benefNoCol])}${String(r[childIdxCol] ?? "")}`
          )
          .filter(Boolean);
        ...
        if ((e as any).code === "SQLITE_CANTOPEN") {
          return NextResponse.json({
            count: 0,
            totalInDb: 0,
            duplicateIds: [],
    ...
    if (action === "save") {
      ...
      (async () => {
        ...
        try {
          ...
          bnfCmamDb = new Database(getBnfCmamDbPath(), { fileMustExist: true });
          projectsDb = new Database(getProjectsDbPath(), {
            fileMustExist: true,
          });
          ...
          const processedRecords = records.map((row: any) => {
            ...
            if (benefRecord) {
              ...
              if (
                regDate &&
                currDate &&
                benefRecord.BENEF_CLASS_DESC === "مستفيدة"
              ) {
          ...
          const insertStmt = childDb.prepare(
            `INSERT INTO child_cmam (${insertCols
              .map((c) => `"${c}"`)
              .join(", ")}) VALUES (${insertPlaceholders})`
          );
          ...
          const transaction = childDb.transaction(() => {
            for (const record of processedRecords) {
              ...
              const checkStmt = childDb.prepare(`SELECT id FROM child_cmam WHERE project_id = ? AND ${uniqueIdCol} = ?`);
              const existing = checkStmt.get(projectId, uid);
              if (existing) {
