...
function generateHTML(childrenForHc: any[], fontBase64: { regular: string, bold: string }, logoBase64: string): string {
  let html = `<html><head><style>${getStyles(fontBase64.regular, fontBase64.bold)}</style></head><body><div class="pdf-wrapper">`;
  ...
  html += `
    <div class="pdf-page">
      <div class="logo-box">
        <img src="${logoBase64}" style="height: 20mm; width: auto;" alt="SFD Logo" />
      </div>
...
export async function POST(req: Request) {
    ...
    try {
        ...
        // Load fonts and logo
        const [fontRegBuffer, fontBoldBuffer, logoBuffer] = await Promise.all([
            fs.readFile(path.join(process.cwd(), 'public/fonts/NotoNaskhArabic-Regular.ttf')),
            fs.readFile(path.join(process.cwd(), 'public/fonts/NotoNaskhArabic-Bold.ttf')),
            fs.readFile(path.join(process.cwd(), 'public/sfd-logo.png'))
        ]);
        
        const fontBase64 = {
            regular: fontRegBuffer.toString('base64'),
            bold: fontBoldBuffer.toString('base64'),
        };
        ...
        if (Object.keys(groups).length === 0) {
            throw new Error("No data groups found to generate PDFs.");
        }
        ...
        if (asZip) {
            ...
            return new NextResponse(zipBuffer, {
                headers: { 'Content-Type': 'application/zip', 'Content-Disposition': `attachment; filename="CMAM_Confirmations.zip"` },
            });
        } else {
            const firstGroupKey = Object.keys(groups)[0];
            const htmlString = generateHTML(groups[firstGroupKey], fontBase64, logoBase64);
            await page.setContent(htmlString, { waitUntil: 'networkidle0' });
            const pdfBuffer = await page.pdf({ format: 'A4', landscape: true, printBackground: true });
            return new NextResponse(pdfBuffer, {
                headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `attachment; filename="CMAM_Sample.pdf"` },
            });
        }
