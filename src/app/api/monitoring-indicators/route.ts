...
const getDbPath = () => path.join(getDataPath(), 'monitoring_indicators.db');
const getJsonPath = () => path.join(getDataPath(), 'monitoring-indicators.json');
...
async function initializeDatabase() {
    ...
    const result = db.prepare('SELECT COUNT(*) as count FROM monitoring_indicators').get() as { count: number };
    const count = result.count;
    if (count === 0) {
        console.log('Seeding monitoring_indicators database from monitoring-indicators.json...');
        try {
            const jsonString = await fs.readFile(getJsonPath(), 'utf-8');
            const plans = JSON.parse(jsonString);
            if (Array.isArray(plans)) {
                const insert = db.prepare(`
                    INSERT OR IGNORE INTO monitoring_indicators (projectId, indicators)
            ...
            console.error("Failed to read or seed monitoring-indicators.json:", error);
        }
    }
    return db;
}
...
export async function GET(req: Request) {
    ...
    try {
        ...
        if (projectId) {
            const plan = db.prepare('SELECT * FROM monitoring_indicators WHERE projectId = ?').get(projectId);
            db.close();
            if (plan) {
        ...
        const plans = db.prepare('SELECT * FROM monitoring_indicators').all();
        db.close();
        return NextResponse.json(plans.map(p => ({ ...p, indicators: JSON.parse(p.indicators) })));
    } catch (err: any) {
        console.error("[MONITORING_INDICATORS_API_GET_ERROR]", err);
        return NextResponse.json({ error: "Failed to read indicator tracking plans database.", details: String(err) }, { status: 500 });
    }
}
...
export async function POST(req: Request) {
    try {
        ...
        const stmt = db.prepare(`
            INSERT INTO monitoring_indicators (projectId, indicators)
            VALUES (@projectId, @indicators)
        ...
        console.error("[MONITORING_INDICATORS_API_POST_ERROR]", err);
