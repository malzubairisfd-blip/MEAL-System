  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const key of keysToCheck) {
      ...
      if (matches.length > COMMON_TOKEN_THRESHOLD) {
          // Optimization: Skip checking massive generic blocks if we have specific matches?
          // For now, we process, but rely on the fact that scoring is fast.
      ...
      for (const idx of matches) {
        if (idx > i) { // Only forward comparisons
          candidates.add(idx);
        }
      }
    }
...
const runClustering = async (rows: PreprocessedRow[], edges: any[], opts: WorkerOptions) => {
  ...
  for (let idx = 0; idx < edges.length; idx++) {
    ...
    if (idx % 2000 === 0) { // Reduced frequency for speed
      postMessage({
        type: "progress",
...
Paste all the content of scoring Worker
...
const records = cluster.records || [];  
  ...
  pairScores.forEach((pair) => {  
    ...
    // Use combined name score for per-record calculation
    const nameScore = (pair.firstNameScore + pair.familyNameScore) / 2;
    entryA.nameScore.push(nameScore);  
    entryB.nameScore.push(nameScore);  
