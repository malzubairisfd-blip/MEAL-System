...
// This worker expects lightweight blocking keys on incoming rows (property: _blockingKeys).
// It uses those blocking keys to reduce the candidate pairs (only compares pairs that share a block key).
// The pairwise scoring logic is unchanged so clustering outputs are preserved where blocking captures the same candidates.
...
type PreprocessedRow = {
  ...
  parts: string[];
  husbandParts: string[];
  blockingKeys?: string[]; // provided by main thread
};
...
self.onmessage = (event) => {
  ...
  if (type === "data") {
    ...
    // Populate block map with provided blocking keys, fallback to first token if none.
  ...
  if (type === "end") {
    setTimeout(async () => {
      try {
        ...
        // Build edges by iterating blocks only and avoiding duplicate pair computations.
        ...
        // Sort blocks by size descending to process large blocks first (heuristic)
        blockEntries.sort((a, b) => b[1].length - a[1].length);

        for (const [blockKey, indices] of blockEntries) {
          // To control work on extremely large blocks, chunk inner loops
