...
const computePairScore = (rowA: PreprocessedRow, rowB: PreprocessedRow, opts: WorkerOptions) => {
    const o = opts;
...
const buildEdges = async (
  ...
  for (; i < n; i++) {
    for (; j < n; j++) {
      const result = computePairScore(rows[i], rows[j], opts);
  ...
  rowsChunk.map((row) => {
    ...
    Object.entries(mapping).forEach(([key, column]) => {
      ...
      if (row[column] !== undefined) {
        mappedRow[key as keyof RecordRow] = row[column];
      }
    });
...
const splitCluster = (rowsSubset: PreprocessedRow[], minInternal: number, opts: WorkerOptions) => {
  ...
  for (let i = 0; i < rowsSubset.length; i++) {
    for (let j = i + 1; j < rowsSubset.length; j++) {
      const result = computePairScore(rowsSubset[i], rowsSubset[j], opts);
