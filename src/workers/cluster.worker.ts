...
const applyAdditionalRules = (a: PreprocessedRow, b: PreprocessedRow, opts: WorkerOptions) => {
  ...
  /* -------------------------------------------------
     2️⃣ STRONG DUPLICATED HUSBAND LINEAGE
  ------------------------------------------------- */
  if (
    A.length >= 3 &&
    B.length >= 3 &&
    ...
    const husbandLineage =
      s93(HA[0], HB[0]) &&
      s93(HA[1], HB[1]) &&
      s93(HA[2], HB[2]) &&
      jw(HA[HA.length - 1], HB[HB.length - 1]) >= 0.9;

    if (womanLineage && husbandLineage) {
      return {
        score: Math.min(1, minPair + 0.30),
  ...
  /* -------------------------------------------------
     3️⃣ WOMAN + HUSBAND LINEAGE MATCH (4–5 PARTS)
  ------------------------------------------------- */
  if (
  ...
  /* -------------------------------------------------
     4️⃣ SAME HUSBAND – WOMAN FAMILY CHANGED
  ------------------------------------------------- */
  if (
    A.length >= 3 &&
    B.length >= 3 &&
  ...
  /* -------------------------------------------------
     5️⃣ TOKEN REORDER (GENERAL BUT IMPORTANT)
  ------------------------------------------------- */
  const tokenRatio = (() => {
  ...
  /* -------------------------------------------------
     7️⃣ INVESTIGATION / PLACEHOLDER RECORDS (LOWEST)
  ------------------------------------------------- */
  const investigationWords = ["تحت", "التحقيق", "مراجعة", "قيد", "موقوف", "غير", "مكتمل"];
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (processed % progressInterval === 0) {
      postMessage({
        ...
        progress: 20 + Math.round(30 * (processed / n)),
        completed: processed,
        total: n,
      });
      await yieldToEventLoop();
...
const runClustering = async (rows: PreprocessedRow[], edges: any[], opts: WorkerOptions) => {
  ...
  const clustersWithRecords = mergedClusters
    .map((cluster) => ({
      ...cluster,
      records: cluster.records
        .map((record: any) => rows.find((r) => r._internalId === record._internalId))
        .filter(Boolean),
    }))
