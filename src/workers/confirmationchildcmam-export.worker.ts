// src/workers/confirmationchildcmam-export.worker.ts
...
self.onmessage = async (event: MessageEvent) => {
  const { children, fontBase64, sample } = event.data
  if (!children?.length) {
    postMessage({ type: "error", error: "No children data provided" })
    return
  }
  ...
  for (const hc of Object.keys(groups)) {
    ...
    // Pass all hc Child to cover page
