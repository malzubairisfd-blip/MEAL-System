...
function drawText(doc: jsPDF, text: string | string[], x: number, y: number, size: number, align: "right" | "center" | "left" = "right", isBold = false) {
  doc.setFont("NotoNaskhArabic", isBold ? "bold" : "normal");
  doc.setFontSize(size);
  doc.setTextColor(0, 0, 0);
  // jsPDF natively handles string arrays by printing them on consecutive lines
  doc.text(text, x, y, { align, baseline: 'middle', lang: 'ar' });
}
...
const generateContractPdf = (doc: jsPDF, educator: Educator, project: Project, funder: string): ArrayBuffer => {
  ...
  const party1Text = `١- الصندوق الاجتماعي للتنمية – فرع صنعاء (هاتف: ${arabicNumber('513821')} ، فاكس ${arabicNumber('513803')})  الرقم المجاني للشكاوى والبلاغات (${arabicNumber('8009800')})، ويمثله: م. محمد حسن غمضان بصفته مدير الفرع ويسمى بهذا العقد الصندوق أوـ (الطرف الأول ) أو الصندوق.`;
  ...
  const drawClause = (title: string, text: string) => {
    ...
    // Automatically wraps without justification
