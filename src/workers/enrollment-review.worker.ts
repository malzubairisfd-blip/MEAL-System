...
self.onmessage = async (event) => {
    ...
    try {
        ...
        const processedRecords = cachedData.map((record: any, index: number) => {
            ...
            if(index % 100 === 0) { // Update progress every 100 records
                 postMessage({ type: 'progress', status: `Analyzing & Preparing Records for Cache... (${index + 1}/${totalRecords})`, progress: currentProgress });
            }
