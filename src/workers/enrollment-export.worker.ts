...
self.onmessage = async (event) => {
    const { records, chartImages, processedData } = event.data;
    if (!records) {
        postMessage({ type: 'error', error: 'No records data provided to export worker.' });
        return;
    }
    
    postMessage({ type: 'progress', status: 'Starting', progress: 5 });

    try {
        const workbook = new ExcelJS.Workbook();
        workbook.creator = "MEAL System";
        
        postMessage({ type: 'progress', status: 'Creating main sheet', progress: 20 });
        ...
        postMessage({ type: 'progress', status: 'Creating disqualified sheet', progress: 40 });
        const disqualifiedSheet = workbook.addWorksheet('Disqualified');
        buildDisqualifiedSheet(disqualifiedSheet, records);

        if (chartImages && processedData) {
            postMessage({ type: 'progress', status: 'Creating dashboard', progress: 70 });
            const dashboardSheet = workbook.addWorksheet('Enrollment Dashboard');
            addDashboardImages(workbook, dashboardSheet, chartImages, processedData);
        }

        postMessage({ type: 'progress', status: 'Finalizing', progress: 95 });
        const buffer = await workbook.xlsx.writeBuffer();
        
        const arrayBuffer = buffer instanceof ArrayBuffer ? buffer : buffer.buffer;
        self.postMessage({ type: 'done', data: arrayBuffer }, [arrayBuffer]);

    } catch (error: any) {
        postMessage({ type: 'error', error: error.message || 'An unknown error occurred during Excel generation.' });
    }
};
